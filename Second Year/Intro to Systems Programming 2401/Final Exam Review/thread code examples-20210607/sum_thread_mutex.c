...
/* 
Purpose: demonstrates usage of mutex in critical code
...
In order to overcome race in writing the sum to the global varabiel totalSum the program is using mutual 
exclusion to ensure that the critical section is accessed by a single thread at a time.  

Each thread in each turn gets exculsive access to the critical section of the code computes the sum and then 
releases the cricial section to aother threads. 
